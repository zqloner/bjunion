        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                for(var i=0;i<perlist.length;i++){
                    ...
                    var datas = {};
                    datas.tempId = new Date().getTime();
                    datas.xuhao = i+1;
                    datas.kind = perlist[i].costType;
                    datas.detail = perlist[i].costinfoid;
                    if(perlist[i].userId == undefined){
                        datas.name = -1;
                    }else {
                        datas.name = perlist[i].userId
                    }

                    datas.num = price;
                    datas.ps = perlist[i].remarks;

                    viewObj.tbData.push(datas);
                    //oldData.push(datas);
                }
                $("#totalmoney").html(totalmoney);
                tablePlayAdd(viewObj.tbData);
            ...
            success: function (res) {
                ...
                if(undefined!=appends&&appends.length>0){
                    for(var i=0;i<appends.length;i++){
                        preview.push(
                            {name:appends[i].NAME,url:appends[i].url}
                        );
                        $("#preview").append('<p class="deP">' +
                            '<span>附件： '+appends[i].NAME+'</span> <a class="deA delete" href="javascript:void(0);" data-href="'+appends[i].url+'" data-name="'+appends[i].NAME+'">删除</a>' +
                            '</p>');
                    }
                }
            }
